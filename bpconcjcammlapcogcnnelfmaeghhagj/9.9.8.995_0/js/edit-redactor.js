...
//                canvas.cache.data = canvas.fon.context.getImageData(0, 0, canvas.common.width, canvas.common.height);
        ...
        this.loadBackgroundImage = function (img, cb) {
            if (typeof img === 'object') {
                const {w, h, z} = nimbus_screen.info.file.image.info;

                const width = Math.ceil(w)
                const height = Math.ceil(h)

                $(_this).width(width).height(h);

                canvas.common.width = width;
                canvas.common.height = height;

                $(canvas.fon.canvas).attr("width", width);
                $(canvas.fon.canvas).attr("height", height);
                $(canvas.current.canvas).attr("width", width);
                $(canvas.current.canvas).attr("height", height);
                $(canvas.background.canvas).attr("width", width);
                $(canvas.background.canvas).attr("height", height);

                canvas.fon.context.fillStyle = "white";
                canvas.fon.context.fillRect(0, 0, width, height);
                canvas.fon.context.drawImage(img, 0, 0, width, height);
        ...
        this.loadImageObject = async function (dataURL, data, cb) {
            const image = await nscExt.imageLoad(dataURL)
            ...
            // image.onload = function () {
                ...
                c.width = data.width || image.width;
                c.height = data.height || image.height;
