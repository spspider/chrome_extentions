...
function setDevices (devices) {
    ...
    if (!mic_is) {
        localStorage.selectedMicrophone = mic_first;
    }
    if (!cam_is) {
        localStorage.selectedVideoCamera = cam__first;
    }
}
...
$(async function () {
        ...
        switch (this.name) {
            ...
            case 'capture-blank':
                ...
                if (this.name === 'capture-entire') {
                    ...
                    if (trialFullPageScreenshotCount > 5 && !premium && isTrialScreenshot) {
                        return window.close()
                    }
    ...
    const appType = nscExt.getOption('appType')
    ...
    if (appType === 'google') {
        $('.nsc-capture-title-logo').addClass('nsc-hide')
    }
