(function () {
  ...
  function checkInjectOverlay() {
    ...
    if (window.self === window.top) {
      ...
      try {
        ...
        debug("This error message was caught so it doesn't clutter up console.log.");
        debug(err);
      }
    }
    return canInject;
    ...
    if (me.id && !structure[me.id]) {
      ...
      witnessMe = () => {
        ...
        global.observer[me.id] = new MutationObserver(function (obs) {
          ...
          if (obs[0].attributeName === 'style' || obs[0].attributeName === 'class') {
  ...
  */
  ...
  function checkImage(me) {
    ...
    [
      ...
      () => {
        if (!me.img.currentSrc.match(/^http/) && !me.img.currentSrc.match(/^data/)) {
          return true;
        }
      },
      ...
      () => {
        if (me.img.getAttribute('nopin') || me.img.getAttribute('data-pin-nopin')) {
          return true;
        }
      },
    ].filter((it) => {
  ...
  */
  ...
  function getImageData(img) {
    ...
    found.description =
      get({ el: img, att: 'data-pin-description' }) || img.title || document.title;
    ...
    if (!global.nopin && !global.nohover) {
      ...
      if (el === structure.buttonSave) {
        ...
        if (
          ...
          global.hoverImage.naturalWidth >= config.hoverBoardPicker.minImageSize &&
      ...
      if (el.tagName === 'IMG') {
        ...
        if (!global.hazLogin && !el.currentSrc.match(/^((http|https):\/\/)/)) {
          ...
          if (!checkImage({ img: el })) {
            ...
            structure.buttonSave.style.top = p.top + config.save.offset.top + 'px';
            structure.buttonSave.style.left = p.left + config.save.offset.left + 'px';
            ...
            if (!global.nosearch) {
              ...
              structure.buttonSearch.style.top = p.top + config.save.offset.top + 'px';
              structure.buttonSearch.style.left = p.right - config.save.offset.right + 'px';
            }
    ...
    [
      ...
      () => {
        ...
        */
        ...
        if (document.URL.match(pinterestMatch) || document.URL.match(pinterdevMatch)) {
              ...
              if (key === 'pinterest') {
                ...
                if (value === 'nopin') {
                  if (meta[i].getAttribute('description')) {
                    global.customNoPinDomain = meta[i].getAttribute('description');
                  }
                }
              }
            }
  ...
  */
  ...
  function init() {
    ...
    if (!checkInjectOverlay()) {
      ...
      structure.buttonSave.setAttribute('style', config.save.style.lego.join('!important;'));
      // make Search button
      structure.buttonSearch = document.createElement('SPAN');
      structure.buttonSearch.setAttribute('style', config.search.style.join('!important;'));
      // handle events
